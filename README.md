...
The `set_current` subroutine also requires a `boolean` as a second parameter. This tells the parser whether it should resolve any datetime overflows in the datetime code you passed. For example, `*gosub_scene clock set_current "1/4/2001 12:34:66" true` would resolve to *April 1st 2001, 12:35:06* because `min_len` is `60` by default.
#### Formatting Marks
...
*NOTE!*  If no formatting marks are used the routine will assume you mean seconds and then set the time accordingly. So if you want to deal exclusively with *unix-like* times, just add numbers and let the parser resolve the overflows.
...
*NOTE!* When setting or travelling in time, formatting marks are ***case-sensitive!***
